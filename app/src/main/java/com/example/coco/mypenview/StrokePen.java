...
public class StrokePen {
    ...
    private ArrayList<ControllerPoint>list;
    private ArrayList<ControllerPoint>mlist;
    ...
    public StrokePen(Context context) {
        list = new ArrayList<ControllerPoint>();
        mlist = new ArrayList<ControllerPoint>();
        mBezier = new Bezier();
    ...
    public void draw(Canvas canvas) {
        ...
        if (mlist.size() < 2) {
            ControllerPoint point = mlist.get(0);
            //由于此问题在算法上还没有实现，所以暂时不给他画圆圈
            //canvas.drawCircle(point.x, point.y, point.width, mPaint);
        } else {
            curPoint = mlist.get(0);
            for (int i = 1; i < mlist.size(); i++) {
                ControllerPoint point = mlist.get(i);
